...
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth,height=100mm,keepaspectratio]{Caso0/PSD_MOTORREDUTOR_FILTRADO}
\caption {Espectro de densidade de potência do motorredutor. Elaborado pelo autor.}
\label{PSD_MOTORREDUTOR_FILTRADO}
\end{figure}

A partir desta filtragem pode-se verificar uma predominância de frequências na região dos {\SI{3800}{\hertz} aos {\SI{4000}{\hertz}. 
...
As caracteristicas do motorredutor, portanto. 

\section{\normalfont{\textbf {Caso 3 - caracterização do sinal do rolamento em bom estado}}}

Para caracterização do sinal do rolamento em bom estado, foram adotados os mesmos métodos anteriormente 
...
Para caracterização do rolamento com dano introduzido no elemento rolante, também foi adotado o método de análise do envelope do sinal. O envelope foi processado em dois conjuntos distintos: 1) no sinal cru, como aquisitado do sensor; 2) no sinal filtrado com as frequências de interesse, conforme visualizado através da análise espectral através do PSD.
...
\section{\normalfont{\textbf {Comparação dos resultados}}}


\chapter{Conclusões}

Neste trabalho foram mostradas evidências da viabilidade de utilização do sensor filme PVDF para detecção de danos em rolamentos. 
