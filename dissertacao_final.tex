...
\subsection{\textbf{Detecção de falhas em rolamentos por análise de vibração}}
...
Dentre as técnicas de análise espectral, a mencionada transformada rápida de Fourier, conhecida pela sigla FFT, é uma das mais largamente empregadas. Entretanto, para que se obtenha resultados mais claros com a aplicação da FFT na análise espectral de rolamentos, dado o mascaramento das frequências dos por outros elementos de máquinas, fazem-se necessárias técnicas que permitam o isolamento do sinal dos rolamentos a fim de que se possa detectar o dano em sua fase inicial. 

Para tanto, são empregados métodos de filtragem de sinais, que serão abordados em maior profundidade na seção~\ref{sub:condicionamento} deste trabalho, bem como de análise no domínio conjunto tempo-frequência, tal como a STFT, \textit{Short Time Fourier Transform}. 

Neste sentido, estas técnicas serão abordadas dentro do método mais largamente empregado para análise de danos em rolamentos: a análise de envelope (KIM et. al, 2020). 

Segundo Randall, 2011 \textit{apud} Almeida, 2013, a análise de envelope da vibração tem como objetivo que seja feita a troca de uma faixa mais ampla das frequências da FFT para uma faixa menor, de forma que isso permita uma análise com maior resolução. Assim, a análise de envelope segue os passos a seguir:
