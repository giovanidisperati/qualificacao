...
\begin{figure}[H]
\centering
\caption {Espectro de densidade de potência do rolamento com dano no elemento rolante. Vista ampliada da região de baixas frequências, onde o pico em 10Hz mostra-se presente.}
...
Uma possibilidade seria a aplicação da demodulação do sinal para análise do envelope. Entretanto, dada a utilização dos circuitos condicionadores com filtro passa-baixa, para diminuição do ruído dos sensores, parte da informação de alta frequência possivelmente portadora do sinal de falhas não se mostra presente no sinal aquisitado. 
...
Percebe-se uma amplitude pico a pico superior a {\SI{400}{\milli\volt}} no sinal, semelhante ao obtido no ensaio do rolamento com dano no elemento rolante. Observando-se o PSD do sinal, é visto pico de frequências na região dos {\SI{1596}{\hertz}, bem como picos de menor amplitude nos {\SI{1405}{\hertz}, {\SI{1198}{\hertz} e {\SI{387}{\hertz}. Observando-se o espectro de baixas frequências, percebe-se um pico aos {\SI{5}{\hertz}, próximo à frequência calculada para o dano na pista externa. 

\begin{figure}[H]
\centering
\caption {Espectro de densidade de potência do rolamento com dano na pista externa.}
\includegraphics[width=\textwidth,height=190mm,keepaspectratio]{PSD_PVDF_rolamento_dano_pista_externa} \\
Fonte: O autor (2022).
\label{DEFEITO_PISTA_EXTERNA}
\end{figure} 

Assim como no caso anterior, as frequências mais elevadas mostraram-se menos presentes dada a utilização do filtro passa-baixa. Contudo, como mencionado e assim como no caso anterior, em que analisou-se o dano no elemento rolante, verifica-se o aparecimento de uma frequência próxima ao esperado para o dano diretamente no espectro de densidade de potência.

\begin{figure}[H]
\centering
\caption {Espectro de densidade de potência do rolamento com dano na pista externa. Vista ampliada da região de baixas frequências, onde o pico próximo aos 5Hz mostra-se presente.}
...
A comparação entre os sinais mostra que o sinal do sensor acelerômetro mostra-se mais bem definido. É importante notar que é apresentado, aqui, o espectro de potência do sensor PVDF uma vez que os ruídos criam um espalhamento por todo o espectro, dificultando a visualização das frequências que foram realmente capturadas. A menor diferenciação das frequências por parte do PVDF é visível nos picos dos {\SI{472}{\hertz} e {\SI{499}{\hertz}.

Nos casos onde ocorrem os danos de rolamentos, porém, o acelerômetro mostrou-se bastante superior. O PVDF aponta concentrações no espectro bastante próximos de onde as frequências base das falhas deveriam estar presentes, contudo, não é possível afirmar taxativamente que estas tratam-se das frequências de falhas dos rolamentos - ainda que ressaltadas apenas nos casos onde os danos são presentes e, portanto, sugerindo se tratar, de fato, das frequências base das falhas. 
...
Percebe-se, assim, que o sensor acelerômetro mostrou-se superior ao PVDF no arranjo experimental adotado, permitindo uma análise de maior acurácia na detecção dos danos nos rolamentos enquanto acoplados ao eixo do motorredutor. Isso se deu, por sua vez, pois foi perceptível a interferência de frequências dos componentes da caixa de redução, que tiveram suas assinaturas vibracionais transmitidas por meio do eixo motor, tendo sido capturadas nas caixas dos rolamentos. Um outro fator a se considerar no experimento é a baixa velocidade de rotação do eixo, que impôs maior dificuldade à análise. Isto se deu pois a baixa rotação, dada a velocidade máxima de transmissão ao eixo pelo conjunto motorredutor, fez com que os impactos causados pelos danos nos rolamentos fosse de menor amplitude e, portanto, de menor propagação quando se comparado a um cenário com maior velocidade de rotação. A detecção das frequências características mesmo quando adotado o sensor acelerômetro não se mostrou óbvia em análise inicial visto que análise dos danos nas condições experimentais foi, em si mesmo, de elevada complexidade. 
...
Neste trabalho buscaram-se evidências da viabilidade inicial da utilização do sensor filme PVDF para detecção de danos em rolamentos autocompensadores de rolos, sob as condições impostas durante os experimentos realizados. Ruídos mostraram-se presentes dada à natureza de alta impedância do sensor PVDF, mas puderam ser mitigados com o desenvolvimento de um conjunto de circuitos condicionadores de sinais e com a usinagem de um encapsulamento de alumínio, que foi construído para acomodação e isolamento do sensor. Após as caracterizações e análises, verificou-se que foi possível diminuir significativamente os níveis de ruídos captados pelo sensor PVDF. 

Também foi elaborado um sistema de aquisição de dados na plataforma LabView, apresentado em apêndice, e desenvolvido um sistema de aquisição baseado no microcontrolador STM32, de modo que este, em conjunto com circuito de condicionamento do sinal, possa vir a ser utilizado para estudos futuros de viabilidade de um sistema composto por sensor PVDF em conjunto com sistema de aquisição de dados de baixo custo.

Além disso, foi construída uma bancada experimental para condução dos ensaios, bem como desenvolvido um conjunto de programas para análise dos sinais do rolamento, tanto no domínio do tempo, quanto no domínio frequência, com os mesmos disponibilizados em apêndice. Tais métodos apoiam-se na literatura específica do tema, apresentada durante a revisão bibliográfica realizada e foram validados com utilização de conjuntos de dados experimentais disponibilizados. 

Por meio dos ensaios percebe-se que a aplicação dos métodos de detecção de danos em rolamentos requer o conhecimento prévio de suas frequências esperadas, não sendo imediatamente óbvia a localização destes no espectro de frequências. A baixa rotação do eixo motriz impôs uma maior complexidade na análise, dadas as baixas frequências onde as frequências características de defeitos se localizavam no espectro. Além disso, conforme verificado na própria literatura, a localização das frequências características de defeito aproxima-se do estimado, mas usualmente não coincide com absoluta precisão. Dadas as vibrações de outros elementos do motorredutor, transmitidas por meio do eixo, percebeu-se que isto representou um desafio na detecção das falhas. Ademais, como visto na literatura, o próprio estágio de degradação do rolamento pode impor um elevado nível de ruído no espectro de frequências e, nesse sentido, ao se trabalhar com aplicações de diagnósticos com falhas introduzidas manualmente, há, ainda, que se ter cautela durante a introdução dos danos. Portanto, a introdução das falhas baseou-se, também, em referências encontradas na literatura.

O sensor filme PVDF, com o arranjo de circuitos condicionadores de sinal e encapsulamento utilizado durante os ensaios, não se mostrou alternativa imediata ao sensor acelerômetro para detecção de danos na pista externa e no elemento rolante de rolamentos autocompensadores de rolos sob as condições experimentais adotadas. A presença de ruídos que fizeram necessária a aplicação de filtro passa-baixa, em conjunto com a baixa rotação do eixo motriz, não permitiu a aplicação da técnica de demodulação do sinal, ou análise de envelope. Tentou-se, assim, detectar os danos por meio da análise espectral por meio dos métodos da FFT e PSD. Os resultados sugerem que os danos puderam ser visualizados em suas frequências base com uso do sensor PVDF, mas os padrões gerados pelas falhas e vistos na literatura não puderam ser confirmados e, portanto, não há como se afirmar taxativamente que as mesmas foram detectadas.

Os resultados obtidos com o sensor PVDF, demonstram, ainda assim, uma representação das frequências compatível com às do sensor acelerômetro em ensaios realizados, particularmente na região abaixo dos {\SI{2500}{\hertz}. Pode-se, assim, investigar em trabalhos futuros a possibilidade de detecção dos danos em rolamentos, com uso do sensor PVDF, em condições de maiores rotações, outros arranjos de cargas e/ou com imposição de sinais de falhas de maior amplitude. Esse fato também aponta para a possibilidade de estudos futuros que verifiquem o emprego do sensor em demais aplicações onde essa faixa de frequências se faça suficiente e em que sua definição não seja fator inviabilizante à utilização do mesmo. A bancada experimental, circuitos condicionadores, encapsulamento e softwares de análise construídos, bem como os ensaios, resultados e conclusões elaborados durante este trabalho são, nesse âmbito, contribuições que se tornam ponto de partida para investigações futuras.
