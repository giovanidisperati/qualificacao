...
Portanto, foi construído um encapsulamento para o sensor, bem como um circuito de condicionamento do sinal e um sistema de software para aquisição de dados a fim da análise das informações capturadas pelo sensor durante os testes na bancada experimental, também construída para este trabalho. Tal bancada experimental foi construída com mancais de rolamentos de rolos, da empresa NSK, acoplados a um eixo em um motoredutor cedido pela empresa parceira WEG/Cestari. Além disso, foi desenvolvida revisão bibliográfica acerca do tema e os métodos de análise de falhas em rolamento foram construídos. 

Buscou-se, assim, a partir dos experimentos e análise dos dados, uma resposta inicial quanto a viabilidade de utilização do filme PVDF como sensor de baixo custo para fins de manutenção preditiva.
...
Dentre as distintas aplicações do PVDF, uma revisão de literatura é desenvolvida com base em Xin et al., 2016,  quanto ao seu uso como sensor de vibrações. 

Os autores apresentam suas aplicações para dispositivos médicos portáteis, monitoramento de saúde estrutural, monitoramento de maquinário, dentre outras.

No que diz respeito à sua aplicação em dispositivos médicos portáteis, a utilização neste  segmento se dá pois a respiração e os batimentos cardíacos, de humanos e de animais, geram vibrações das superfícies corporais. Estas vibrações, em diversos casos, refletem as condições de saúde, podendo ser medidas através de sensores colocados na pele. Neste sentido os sensores PVDF têm como vantagem característica sua flexibilidade, baixo custo e leveza. Isto o torna bom candidato para a aplicação nesta área da saúde e muitos pesquisadores têm feito esforços em relação ao seu estudo e desenvolvimento (Xin et al., 2016).
...
Já em relação ao PVDF como sensor aplicado a monitoramento de integridade estrutural, o PZT, titanato zirconato de chumbo, é o material mais comum para monitoramento de integridade de estruturas. Contudo, este material é rígido e não pode ser aplicado em superfícies curvas (Xin et. al, 2016). Disto decorrem os desenvolvimentos do PVDF como material alternativo, uma vez que este pode ser uma resposta para aplicações específicas dada sua flexibilidade. Desta forma, o PVDF é estudado em aplicações em ensaios não destrutivos de detecção de emissão acústica, detecção precoce de danos em lâminas de turbina eólica, determinação da localização e severidade de danos sísmicos em estruturas de aço de edifícios.

Neste sentido, Brown e Mason, 1996, demonstraram que o PVDF é sensível a impactos, podendo ser utilizados em aplicações de ensaios não-destrutivos de emissões acústicas, mas que as propriedades dielétricas e eletromecânicas apresentavam variação com temperatura, o que deveria ser considerado para tais aplicações de monitoramente de integridade estrutural. 
...
Quanto ao PVDF como sensor aplicado a medição de vibrações em maquinário, por sua vez, materiais cerâmicos, como o próprio PZT, costumam ser encontrados nestas aplicações por possuírem maior nível de atividade piezoelétrica. Benech et. al, 1996, entretanto, apresentaram os princípios e \textit{design} da medição de vibrações com o sensor PVDF e concluíram que este apresenta, nesta área, potencial de utilização em aplicações de baixo custo. Os  autores afirmam que a vantagem deste sensor está em sua resistência em relação aos materiais cerâmicos, demonstrando também que o PVDF pode ser utilizado para medições de vibrações em que o custo fosse um parâmetro importante. 
...
Rolamentos são um dos elementos mais utilizados em máquinas e suas falhas, também, são uma das causas comuns para problemas nestes maquinários. Assim, os sinais de rolamentos, em particular os de vibração, têm sido amplamente estudados. 

Randall e Antoni, 2011, apontam que as falhas de rolamentos geram frequências características em seus distintos componentes, relacionadas às dimensões e geometria do rolamento. Isto ocorre pois a superfície do componente danificado no rolamento gera, ao entrar em contato com outra superfície, um impulso que se propaga através de um choque. Tais impulsos se dão de maneira periódica, com características dependentes da localização do defeito e amplitude dependente de sua posição em relação à zona de carga.  

A Figura~\ref{frequencias_sinais_rolamentos} mostra os sinais típicos de aceleração produzidos por falhas localizadas. 
...
\begin{figure}[H]
\centering
\caption {Análise de Envelope com etapas de pré-processamento do sinal. Retirado de Kim, 2020.}
...
Para que possa analisar os dados provenientes de sensores é necessário um sistema de aquisição de dados. França, 1997, define sistema de aquisição de dados afirmando que este trata-se de um conjunto de elementos interrelacionados, os quais visam aquisição, análise e apresentação dos dados relativos ao processo de interesse.
...
\begin{equation}
	FTF = \frac{37}{2} \left ( 1 - \frac{10.1}{51} \,cos \, \ang{14} \right )  \ = \SI{~15}{{\hertz}}
...
\begin{equation}
	BPFI = \frac{10}{2}37 \left ( 1 + \frac{10.1}{51} \,cos \, \ang{14} \right )  \ = \SI{~220}{{\hertz}}
...
\begin{equation}
	BPFO = \frac{10}{2}37 \left ( 1 - \frac{10.1}{51} \,cos \, \ang{14} \right )  \ = \SI{~141}{{\hertz}}
...
\begin{equation}
	BSF = \frac{51}{2*10.1}37 \Bigg[  1 - \left ( \frac{10.1}{51} \,cos \, \ang{14} \right ) ^{2}  \Bigg] \ = \SI{~91}{{\hertz}} \
	\label{eq: BSF no rolamento}
\end{equation}

%\[BSF = \frac{51}{2*10.1}35 \Bigg[  1 - \left ( \frac{10.1}{51} \,cos \, \ang{14} \right ) ^{2}  \Bigg] \ = \SI{~85}{{\hertz}} \]

O conhecimento prévio destas frequências é de grande importância na etapa de análise, uma vez que este 
...
Este trata-se de um filme PVDF com espessura de \SI{28}{\micro\meter}, laminado em folha de poliéster e com dois terminais crimpados, que permitem a medição de sua tensão quando sujeito a um esforço mecânico. Este tem como principais características: 
...
Tendo em vista se tratarem de sensores de alta impedância, se faz necessário isolamento elétrico, a fim de mitigar as interferências da fase da rede elétrica e de outros sinais de alta frequência incidentes sobre o local de testes, bem como amplificação da tensão resultante do esforço mecânico através de amplificador operacional. 

Li et. al, 2011, explicitam o tratamento eletrônico para trabalho com o sensor PVDF, onde mostram a importância de um filtro rejeita-faixa e de amplificação operacional. Também os autores apontam que dada esta alta impedância, este se torna facilmente sujeito aos ruídos ambientes - tal como verificado experimentalmente nos testes realizados neste trabalho.

Portanto, a fim de mitigar este problema, a montagem dos sensores se deu em um encapsulamento de alumínio para diminuição destes ruídos. Os sensores foram acoplados com cola acrílica, a fim de que não ocorresse amortecimento significativo da vibração devido ao elemento colante. A Figura~\ref{encapsulamento_sensor} mostra uma montagem do sensor em seu encapsulamento.
...
A Figura ~\ref{sensor_e_operacional} mostra o sensor e o circuito amplificador operacional, montados no encapsulamento de alumínio, onde se acomoda o filme PVDF, além de amplificador operacional Texas Instruments TL082.
...
Além destes, conforme mencionado, também foram desenvolvidos filtro passa-baixa e filtro rejeita-faixa com regulagem de frequências específicas para os problemas encontrados

A Figura~\ref{amplificador_filme_pvdf} mostra o desenho esquemático do circuito de condicionamento do sinal do sensor piezoelétrico. Neste buscou-se a amplificação do sinal, tendo se optado pelo amplificador operacional de propósito geral Texas Instruments TL082, uma vez que este possui dois canais e larga banda de \SI{1}{\mega\hertz} (Texas Instruments, 2021). Os valores dos componentes deste circuito serão alterados a partir das informações obtidas nos testes.

\begin{figure}[H]
\centering
\caption {Circuito amplificador operacional do sensor piezoelétrico.}
\includegraphics[width=\textwidth,height=\textheight,keepaspectratio]{amplificador_filme_pvdf} \\
Fonte: O autor (2022).
\label{amplificador_filme_pvdf}
\end{figure} 
...
Os capacitores foram ligados em série para atingir os valores adequados e medidos com diferença inferior a 0.5\% entre si, de modo a fornecer melhor resultado neste processo de filtragem. A Figura~\ref{circuito_notch} apresenta o desenho esquemático do circuito rejeita-faixa.
...
A Figura~\ref{sinal_filtrado_maquina_desligada} compara os sinais pré-filtragem e pós-filtragem do sensor: em vermelho, o sinal saído diretamente do amplificador operacional sem filtragem; em branco, o sinal filtrado pelo filtro rejeita-faixa. 
...
Uma vez montado o aparato, os sinais decorrentes da vibração foram aqsuisitados para posterior comparação com os sinais característicos dos rolamentos sem defeitos a fim de que se determine a severidade da falha. Para tanto, foram desenvolvidos sistemas de aquisição de dados. 
...
Além disso também foi construído um sistema de aquisição baseado no microcontrolador STM32, modelo stm32f103, também conhecido como \textit{bluepill}. Este foi utilizado por se tratar de um microcontrolador largamente disponível no mercado, de baixo custo, com \SI{72}{\mega\hertz} de velocidade de CPU e 20KB de memória RAM, também possuindo conversor analógico digital com resolução de 12 bits. 
...
O software foi desenvolvido em linguagem C, com utilização do sistema operacional FreeRTOS, manipulação dos registradores do microcontrolador e testado com dispositivo gerador de função. A construção deste justificou-se como um experimento a fim de se verificar a viabilidade conjunta de um sistema de aquisição de baixo custo em conjunto com o sensor de baixo custo. Em conjunto com o sistema de comunicação em linguagem C, desenvolvido para o microcontrolador, foi desenvolvido também um sistema de registro dos dados obtidos através de comunicação serial em Python, capaz de registrar os dados aquisitados e gravá-los em arquivos. Ambos estes softwares encontram-se também no Apêndice~\ref{secao:programastm}.
...
Para análise dos dados obtidos foram desenvolvidos programas na plataforma MatLab e Python. Foram adotados os métodos descritos durante a Revisão Bibliográfica, em particular a densidade espectral de potência, nível RMS, FFT, e Análise de Envelope com utilização de pré-processamento dos dados com modelo de autoregressão e seleção de banda por meio de Curtose Espectral. O código para análise de Envelope baseou-se no disponibilizado por Kim et al., 2020. Os códigos para estes programas de análise dos dados pode ser encontrado no apêndice ~\ref{secao:programasAnalise}.

Para construção destes foi utilizado a plataforma Anaconda e o Jupyter Notebook de Python, bem como também foi utilizado para teste dos códigos a plataforma Octave, alternative livre ao sistema Matlab, sendo possível verificar o funcionamento dos métodos de análise em ferramentas de software livre, com licenças do tipo BSD. 

\section{\normalfont{\textbf{Definição dos experimentos}}}

A definição dos experimentos se deu a partir de diferentes cenários de caracterização dos sinais. Estes deram pela necessidade de avaliação do sinal gerado pelo sensor, acoplado à caixa de rolamento, enquanto sujeito a diferentes níveis de interferências eletromagnéticas, pois, como descrito na seção ~\ref{secao:CircuitosCondicionadores} há uma quantidade perceptível de interferências eletromagnéticas no sensor por sua natureza de alta impedância.
...
Para condução da aquisição destes distintos cenários as taxas de aquisição foram baseadas nos modelos encontrados em outros artigos, tais como Kim et al., 2020. Estas taxas respeitam o critério de Nyquist para representação do espectro de frequências de interesse. Optou-se pela captura, nas taxas de aquisição descritas na Tabela~\ref{tab:conjuntoDeTestes}, por dois minutos ininterruptos em cada caso –tempo suficiente para captura de um conjunto de rotações do eixo e, portanto, da assinatura dos danos nos rolamentos. 
...
\begin{figure}[H]
\centering
\caption {Dano introduzido no elemento rolante.}
\includegraphics[width=\textwidth,height=80mm,keepaspectratio,angle=-90]{dano_introduzido_rolante} \\
Fonte: O autor (2022).
\label{dano_introduzido_rolante}
\end{figure}

Já a Figura~\ref{insercao_dano_pista_externa} mostra o processo de inserção de dano na pista externa, realizado com uma furadeira de coluna.
...
\begin{figure}[H]
\centering
\caption {Pista externa com dano inserido.}
...
É possível verificar, neste caso, o aumento da amplitude pico a pico dos sinais, superior a {\SI{250}{\milli\volt}}. Já em relação ao sinal espectral deste caso, como é possível visualizar na Figura~\ref{PSD_SENSOR_MAQUINA_LIGADA_E_ENERGIZADA}, o mesmo aponta picos na região dos {\SI{60}{\hertz} e dos {\SI{9500}{\hertz}. Nota-se, entretanto, que ao contrário do experimento anterior, onde as interferências de alta frequência dominavam o espectro em termos de amplitude, neste experimento a frequência da corrente alternada volta a ser dominante. Isto ocorre pela indução causada pelos elementos da bancada de testes, agora energizados, tal como o próprio motorredutor e o inversor de frequências.   
...
Nota-se que a amplitude dos sinais aumenta consideravelmente quando a bancada está conectada à rede elétrica. Isto demonstra, por um lado, que há baixo indução de ruídos causada pela própria fonte de alimentação do circuito condicionador de sinal. Por outro lado, há uma quantidade de ruído advinda do próprio ambiente. Brisebois, 2006, aponta que sensores de alta impedância podem sofrer ruídos por uma série de fatores, tais como ruído externo, resíduos de fluxo de solda, correntes de polarização, dentre outros. Uma vez caracterizados tais ruídos, entretanto, eles podem ser considerados ao se analisar os resultados para verificação da viabilidade do filme PVDF na aplicação proposta.
...
A partir destas caracterizações, é dado prosseguimento nos experimentos a fim de identificação de danos no rolamento. Neste experimento foi introduzido dano no elemento rolante, conforme mostrado na seção~\ref{secao:MateriaisEMetodos}. O dano imposto ao rolamento simulou a situação de esmagamento e foi tal que o ruído do rolamento tornou-se audível ao aproximar-se de onde este estava localizado. 
...
\begin{itemize}
	...
	\item Investigação de métodos de detecção automática de falhas baseado em inteligência artificial, com classificação automática de falhas através da utilização das ferramentas de análise espectral do sinal. 
